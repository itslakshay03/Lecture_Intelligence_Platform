...
def process_youtube_video(task_id: str, url: str, force_refresh: bool = False):
    """
    ...
    """
    logger.info(f"Starting background processing for task: {task_id} (force_refresh={force_refresh})")
    try:
        try:
            update_task(task_id, status="processing", message="Initializing task processing...")
        ...
        try:
            ...
            update_task(task_id, status="failed", error=str(ve), message="Invalid YouTube URL.")
            return
        ...
        update_task(task_id, status="processing", message="Fetching video transcript (Step 1/3)...")
        ...
        update_task(task_id, status="processing", message="Generating study notes with AI (Step 2/3)...")
        ...
        update_task(task_id, status="processing", message="Rendering PDF document (Step 3/3)...")
        ...
        try:
            update_task(task_id, status="failed", error=str(ve), message="Task processing failed.")
        except Exception as e:
        ...
        try:
            update_task(task_id, status="failed", error=f"Task execution failed: {type(e).__name__}: {str(e)}", message="Task execution error.")
...
def start_notes_generation(request: YouTubeRequest, background_tasks: BackgroundTasks):
    """
    ...
    logger.info(f"Received request for URL: {request.url}. Created task: {task_id}")
    
    try:
        create_task(task_id, status="pending", url=request.url)
    except Exception as e:
    ...
    thread = threading.Thread(
        ...
        args=(task_id, request.url, request.force_refresh),
        daemon=True
    )
    thread.start()
...
def get_task_status(task_id: str):
    ...
    logger.info(f"Fetching status details for task: {task_id}")
    try:
    ...
    if not task:
        logger.warning(f"Task {task_id} not found in SQLite.")
...
def download_pdf(task_id: str):
    ...
    logger.info(f"File download request for task: {task_id}")
    try:
        task = get_task(task_id)
    except Exception as e:
